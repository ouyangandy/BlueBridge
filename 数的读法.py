...
def read_four_num(n):
    ...
    if temp[2] != 0 and temp[2] != 1:    #十位不是零和一
        ...
        res.append('shi')
    elif (temp[1] == 0 and temp[2] == 0) or (temp[2] == 0 and temp[3] == 0):   #如果百位是零，十位也是零，连零读一个                                                                    #如果百位个位都是零，不读
        pass
    else:
        res.append(s[temp[2]])
...
if 0 < len(input_data) <= 4:
    ans = read_four_num(input_data.rjust(4,'0'))#不足4位，右对齐补0
elif 4 < len(input_data) <= 8:
    new = input_data.rjust(8,'0')
    read_four_num(new[:4])
    res.append('wan')
    if new[4:] != '0000' and new[4:][0] == '0':
        res.append('ling')
    ans = read_four_num(new[4:])
elif 8 < len(input_data) <= 10:
    new = input_data.rjust(12,'0')
    read_four_num(new[:4])
    res.append('yi')
    if new[4:8] != '0000':
